    ...
    <div id="cdh-content">
        <table class="cdh-table">
            <tr>
                ...
                <th>COBRADO NO PERÍODO</th>
            </tr>
<?php 
        if(isset($_GET['type']) && $_GET['type'] == 1)
            $query = mysql_select_multi("SELECT `id` FROM `pagamentos` ORDER by `agendamento_id` ".getOrder($_GET['order'])." LIMIT $ppage, $rowsPerPage;");
        elseif(isset($_GET['type']) && $_GET['type'] == 2)
            $query = mysql_select_multi("SELECT `id` FROM `pagamentos` ORDER by `valor_total` ".getOrder($_GET['order'])." LIMIT $ppage, $rowsPerPage;");
        elseif(isset($_GET['type']) && $_GET['type'] == 3)
            $query = mysql_select_multi("SELECT `id` FROM `pagamentos` ORDER by `tipo_pagamento` ".getOrder($_GET['order'])." LIMIT $ppage, $rowsPerPage;");
        elseif(isset($_GET['date']) && (!empty($_GET['date'])))
            $query = mysql_select_multi("SELECT `id` FROM `pagamentos` where `data_pagamento`='".$_GET['date']."' ORDER by id ".getOrder($_GET['order'])." LIMIT $ppage, $rowsPerPage;");
        else
            $query = mysql_select_multi("SELECT `id` FROM `pagamentos` ORDER by id ".getOrder($_GET['order'])." LIMIT $ppage, $rowsPerPage;");
                
        if($query) {
            foreach($query as $key => $value) {
                $pay_ID = $value['id'];
?>              
                <tr style="background-color: rgb(85 192 221);" onclick="location.href='cashierHistory.php?subpage=schedulingEdit&data=<?php echo getSchedulingUserID(getPaymentScheduleID($pay_ID)); ?>&scheduling=<?php echo getPaymentScheduleID($pay_ID); ?>'">
                    <td><?php echo $value['id']; ?></td>
                    <td><?php echo getUserCompleteName(getSchedulingUserID(getPaymentScheduleID($pay_ID))); ?></td>
                    <td><?php echo getPaymentValue($pay_ID); ?></td>
                    <td><?php echo getPaymentType($pay_ID) ?></td>
                    <td><?php echo getPaymentDate($pay_ID).' - '.getPaymentTime($pay_ID) ?></td>
                    <td><?php echo getUserCompleteName(getCashierOpeningCollaborator($pay_ID)); ?></td>
                    <td></td>
                </tr>
<?php
            }
    ...
    include 'layout/overall/header.php';
        ...
        **********************************************************/
        if (empty($_POST) === false && getToken('tokenConfirmDeleteSchedule') && empty($errors) && ($_GET["del"] === 'true')) {
            ...
            header("refresh: 2, cashierHistory.php?subpage=userviewer&data=".$_GET['data']."");
        } 
        elseif (empty($_POST) === false && getToken('tokenDeleteSchedule') && empty($errors) && ($_GET["del"])) {
            ...
            <form action='cashierHistory.php?subpage=schedulingEdit&data=".$_GET['data']."&scheduling=".$_GET['scheduling']."&del=true' method='POST'>
                ...
                <a href='cashierHistory.php?subpage=schedulingEdit&data=".$_GET['data']."&scheduling=".$_GET['scheduling']."'>
        ...
        **********************************************************/
        ...
        if(isSchedulingExist($schedule_ID)) {
            $ar_query = mysql_select_single("SELECT `id`, `data_confirmacao`, `observacao` FROM `atendimentos_realizado` where `agendamento_id`= '".$schedule_ID."';");
            $disabled = (isSchedulingServicePerformed($schedule_ID)) ? 'disabled' : '';
            $hidden = (isSchedulingServicePerformed($schedule_ID)) ? 'hidden' : '';
?>
