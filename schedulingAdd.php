    ...
    include 'layout/overall/header.php';
        ...
        **********************************************************/
        if (empty($_POST) === false && getToken('tokenScheduling')) {
            $required_fields = array('cpf', 'service1', 'collaborator', 'dateSchedule', 'hours');
        ...
        **********************************************************/
        if (empty($_POST) === false && getToken('tokenScheduling') && empty($errors)) {
            ...
            header('refresh: 2, schedulingList.php');
...
?>
<center>
    <form action="schedulingAdd.php" method="POST">
        CPF do Paciente:<br/>
        <input type="text" class="mediumInputTxt" name="cpf"></input>
        <br/><br/>
        Escolha o tipo de serviço:<br/>
        <select class='bigSelectInp' name="service1">
            <?php 
                foreach(doListServices(1) as $key => $value) {
                    echo '<option value="'.$value['id'].'">'.$value['descricao'].' '.$value['valor'].'</option>';
                }
            ?>
